...
public class ThreeDMedia extends Application {
  ...
  public void start(Stage stage) {
    ...
    Group group = new Group(box, pointLight, ambientLight);

    // create a 3D scene with a default perspective camera.
    Scene scene = new Scene(group, SCENE_W, SCENE_H, true, SceneAntialiasing.BALANCED);
  ...
  private void showMediaOnShape3D(Shape3D shape3D, final MediaView mediaView) {
    ...
    Scene mediaScene = new Scene(new Group(mediaView), MEDIA_W, MEDIA_H);
    SnapshotParameters snapshotParameters = new SnapshotParameters();
    snapshotParameters.setViewport(new Rectangle2D(0, 0, MEDIA_W, MEDIA_H));
    WritableImage textureImage = mediaView.snapshot(snapshotParameters, null);
    ...
    AnimationTimer timer =
        new AnimationTimer() {
          ...
          public void handle(long now) {
            mediaView.snapshot(snapshotParameters, textureImage);
          }
        };
    timer.start();
  }
  ...
  private void rotateAroundYAxis(Shape3D shape3D) {
    RotateTransition rotateY = new RotateTransition(Duration.seconds(10), shape3D);
