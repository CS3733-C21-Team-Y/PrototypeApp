...
public class ServiceRequestManagerSubpageController {
    ...
    private void pageButtonClicked(ActionEvent e) {
        ...
        if (e.getSource() == laundryBtn)
            stage.setUserData(new StageInformation(((StageInformation) stage.getUserData()).getCenterPaneFXML(), "LaundrySubPage.fxml"));
        else if (e.getSource() == maintenanceBtn)
            stage.setUserData(new StageInformation(((StageInformation) stage.getUserData()).getCenterPaneFXML(), "MaintenanceSubPage.fxml"));
